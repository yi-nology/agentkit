// Package fence 提示词数据区围栏（agent 输入卫生原语）。
//
// 把不可信内容（diff/文件内容/PR 描述/外部工具返回等）包进显式数据区，
// 并中和内容中出现的围栏标记序列（插入空格破坏 token）——防止不可信文本
// 伪造"数据区结束"把后续注入文本抬出数据区、以数据身份获得指令待遇。
//
// 中和发生即注入特征信号：返回值给出中和次数，由调用方留痕/打点
// （fence 自身零依赖、零副作用，可观测策略归调用方）。
package fence
